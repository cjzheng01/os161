...
void sys__exit(int exitcode) {
  ...
  size_t i = 0;
  while (i < array_num(p->children)){ // do not update the index
  ...
  /* if this is the last user process in the system, proc_destroy()
	...
	if (can_fully_delete == 0) {  // set the proc as dead
	    ...
	    pid_t *retval)
{
  ...
  for (size_t i = 0; i < array_num(curproc->children); i++){
    ...
    if (child->pid == pid) {
        ...
        while (child->status) cv_wait(child->parent_cv, child->lk);
...
{
    int err = 0; // record the error code in this function.
    ...
    *retval = child->pid;
    ...
    err = array_add(curproc->children, child, NULL);
...
sys_execv(const char *program, char **args)
{
    ...
    struct addrspace *as;
	struct vnode *v;
	vaddr_t entrypoint, stackptr = USERSTACK;
	int result;
